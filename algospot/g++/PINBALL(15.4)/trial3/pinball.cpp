...
void get_input(){
	scanf("%lf %lf %lf %lf %d", &StartPos.x, &StartPos.y, 
		&StartDir.x, &StartDir.y, &N);
	for(int i=0; i<N; ++i){
		scanf("%lf %lf %lf", &Center[i].x, &Center[i].y, &R[i]);
		R[i] += 1.0;
	}
}

// return next colliding circle index
// if no collision, return -1
int get_next_pos(vector2 la, vector2 lb, int prev_i, vector2& na, vector2& nb){
	vector2 la_lb = lb-la;
	double mindist = INF;
	int mindist_i = -1;
	vector2 mindist_collision;
	...
	for(int i=0; i<N; ++i){
		...
		vector2 pf = perpendicularFoot(Center[i],la,lb);
		vector2 la_pf = pf-la;

		// printf("pf: %lf %lf\n",pf.x,pf.y);
		// printf("la_pf: %lf %lf\n",la_pf.x,la_pf.y);

		if((la_lb.x*la_pf.x) < 0 || (la_lb.y*la_pf.y) < 0){
			continue;
		}

		vector2 center_to_pf = pf-Center[i];
		double ctpd = center_to_pf.norm();

		// printf("ctpd: %lf R[%d]: %lf\n", ctpd, i, R[i]);

		if(ctpd >= R[i])
			continue;

		double cdist = sqrt((R[i]*R[i])-(ctpd*ctpd));
		// printf("cdist: %lf\n",cdist);

		vector2 delta = (pf-la).normalize() * cdist;
		// printf("delta: %lf %lf\n",delta.x,delta.y);

		vector2 collision = pf - delta;

		// printf("pf: %lf %lf\n",pf.x,pf.y);
		// printf("collision: %lf %lf\n",collision.x,collision.y);

		double dist_to_collision = (collision-la).norm();

		// printf("dist_to_collision[%d]: %lf\n",i,dist_to_collision);

		if(dist_to_collision < mindist){
			mindist = dist_to_collision;
			mindist_i = i;
			mindist_collision = collision;
		}
	}

	if(mindist_i != -1){
		na = mindist_collision;
		vector2 pf = perpendicularFoot(la, Center[mindist_i], mindist_collision);
		vector2 delta = pf-la;
		// printf("mindist_i pf: %lf %lf\n",pf.x,pf.y);
		// printf("mindist_i delta: %lf %lf\n",delta.x,delta.y);
		nb = pf+delta;
	}
...
void solve(){
	int prev = -1;
	vector2 la = StartPos;
	vector2 lb = StartPos+StartDir;
	vector2 na;
	vector2 nb;

	for(int i=0; i<100; ++i){
		int col = get_next_pos(la,lb,prev,na,nb);
		if(col == -1){
			break;
		}

		// printf("col: %d  na.x:%lf na.y:%lf nb.x:%lf nb.y:%lf\n",col,na.x,na.y,nb.x,nb.y);
		printf("%d ", col);
		prev = col;
		la = na;
		lb = nb;
