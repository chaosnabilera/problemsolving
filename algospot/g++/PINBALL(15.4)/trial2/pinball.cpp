...
public:
    ...
    vec2& operator-=(vec2 rhs) { x-=rhs.x; y-=rhs.y; return *this; }
    vec2  operator<(vec2 rhs)  { return (x != rhs.x) ? x < rhs.x : y < rhs.y; }
...
bool vec2::segment_intersection(vec2 la, vec2 lb, vec2 ma, vec2 mb){
    ...
    double ccw2 = ccw(ma,mb,la)*ccw(ma,mb,ma);
    if(ccw1 == 0 && ccw2 == 0){
...
void get_input(){
    scanf("%lf %lf %lf %lf %d", &iX, &iY, &dX, &dY, &N);
    for(int i=0; i<N; ++i){
...
void solve(){
    ...
    last_mi = -1;
    for(int t=0; t<100; ++t){
        // printf("(%lf,%lf) (%lf,%lf) (%lf,%lf)\n", pos.x, pos.y, dst.x, dst.y, dir.x, dir.y);
        min_dist = BIGNUM;
        mi = -1;
        ...
        for(int i=0; i<N; ++i){
            ...
            vec2 pf = vec2::perpendicular_foot(P[i],pos,dst);
            double pfl = (pf-P[i]).length();
    
            if(pfl >= R[i])
                continue;
    
            double pf_to_contact = sqrt(R[i]*R[i] - pfl*pfl);
            vec2 contact = pf - ((pf-pos).normalize() * pf_to_contact);
            vec2 dir_to_contact = contact-pos;

            if(dir.x*dir_to_contact.x < 0 || dir.y*dir_to_contact.y < 0)
                continue;

            double dist_to_contact = dir_to_contact.length();
            if(dist_to_contact < min_dist){
                min_dist = dist_to_contact;
                md_contact = contact;
                mi = i;
            }
        }
        ...
        printf("%d ", mi);
        vec2 reflect_center = vec2::perpendicular_foot(pos, P[mi], md_contact);
        vec2 reflect_dst = reflect_center + (reflect_center-pos);
