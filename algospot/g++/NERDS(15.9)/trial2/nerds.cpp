...
void get_input(){
    ...
    for(int i=0; i<N; ++i){
        scanf("%d %lf %lf", &t, &x, &y);
        if(t == 1)
            Nerd.push_back(vec2(x,y));
        else
            Plain.push_back(vec2(x,y));
    }
}

polygon convex_hull(vector<vec2>& V){
    polygon ret;

    vec2 ld = *min_element(V.begin(),V.end());
    ret.push_back(ld);

    while(true){
        vec2 last = ret.back();
        vec2 cand = V[0];
        double max_dist = (cand-last).length();

        for(int i=1; i<V.size(); ++i){
            double ccw_res = vec2::ccw(last, cand, V[i]);
            if(ccw_res > 0 || (ccw_res == 0 && (V[i]-last).length() > max_dist)){
                max_dist = (V[i]-last).length();
                cand = V[i];
            }
        }
        if(cand == ld)
...
void solve(){
    // printf("a?\n");
    polygon ch_nerd = convex_hull(Nerd);
    // printf("b?\n");
    polygon ch_plain= convex_hull(Plain);
    // printf("c?\n");

    if(vec2::inside_polygon(ch_nerd[0],ch_plain) || vec2::inside_polygon(ch_plain[0],ch_nerd)){
        printf("THEORY IS INVALID\n");
        return;
    }

    for(int i=0; i<ch_nerd.size(); ++i){
        vec2 n1 = ch_nerd[i];
        vec2 n2 = ch_nerd[(i+1)%ch_nerd.size()];

        for(int j=0; j<ch_plain.size(); ++j){
            vec2 p1 = ch_plain[j];
            vec2 p2 = ch_plain[(j+1)%ch_plain.size()];

            if(vec2::segment_intersection(n1,n2,p1,p2)){
                printf("THEORY IS INVALID\n");
                return;
            }
        }
