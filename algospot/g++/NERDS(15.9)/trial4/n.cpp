...
void convex_hull(vector<vector2>& points, vector<vector2>& result){
    vector<int> ridx;
    int cur = -1;
    ...
    for(int i=1; i<points.size(); ++i){
        if( (points[cur].x < points[i].x) || 
            (points[cur].x == points[i].x && points[cur].y < points[i].y))
                cur = i;
    }
    ridx.push_back(cur);
    ...
    while(true){
        ...
        for(int i=0; i<points.size(); ++i){
            ...
            double ccw_res = ccw(points[l],points[cand],points[i]);
            if(ccw_res > 0){
                cand = i;
            }
            else if(ccw_res == 0){
                if(howMuchClose(points[l],points[cand],points[i]) > 0)
                    cand = i;
            }
        }
        if(cand == ridx[0])
...
bool solve(){
    ...
    if(isInside(Ach[0], Bch) || isInside(Bch[0],Ach)){
        return false;
    }

    for(int a1=0; a1<Ach.size(); ++a1){
        int a2 = (a1+1) % Ach.size();
        for(int b1=0; b1<Bch.size(); ++b1){
            int b2 = (b1+1) % Bch.size();
            if(segmentIntersects(Ach[a1],Ach[a2],Bch[b1],Bch[b2]))
                return false;
        }
    }
